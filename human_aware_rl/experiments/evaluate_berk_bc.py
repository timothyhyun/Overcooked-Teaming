...
from overcooked_ai_py.utils import load_pickle, save_pickle, load_dict_from_file, profile, load_pkl
...
def test_exp_1_config():
    ...
    TIMESTAMP_DIR = True
    EX_NAME = "testing_ppo_bc_train_simple"
    ...
    MINIBATCHES = 10 if not LOCAL_TESTING else 1
...
def evaluate_other_agent(params, gym_env, mlp, mdp):
    ...
    # best_bc_model_evaluations = load_pickle(BC_MODELS_EVALUATION_PATH)
    # print('best_bc_model_evaluations', best_bc_model_evaluations)

    BC_EXTERNAL_SAVE_DIR = "../../data/bc_runs/"
    BC_EXTERNAL_SAVE_DIR = "../data/bc_runs/"

    best_model_acc_file = BC_EXTERNAL_SAVE_DIR + bc_model_path + '/' + 'best_acc'
    best_model_loss_file = BC_EXTERNAL_SAVE_DIR + bc_model_path + '/' + 'best_loss'
    best_model_metadata_file = BC_EXTERNAL_SAVE_DIR + bc_model_path + '/' + 'bc_metadata'

    # print('')
    # For simple layout
    # best_model_acc = load_pkl(best_model_acc_file)
    # print('best_model_acc  = ', best_model_acc)
    # print()
    #
    # best_model_loss = load_pkl(best_model_loss_file)
    # print('best_model_loss  = ', best_model_loss)
    # print()

    best_model_metadata = load_pickle(best_model_metadata_file)
    print('best_model_metadata  = ', best_model_metadata)
    print()
...
# @ex.automain
# @profile
def evaluate_bc_in_ppo_run(params):
    ...
    for seed in params["SEEDS"]:
        ...
        evaluate_other_agent(params, gym_env, mlp, mdp)
...
################################################
# TESTING CODE HELPER FUNCTIONS
################################################

def get_params_test_exp_1_config():
    #     input_params = {
    #         'EX_NAME': "ppo_bc_train_simple",
    #         'layout_name': "simple",
    # 'REW_SHAPING_HORIZON': 1e6,
    ...
    TIMESTAMP_DIR = True
    EX_NAME = "testing_ppo_bc_train_random3"
    ...
    # Total environment timesteps for the PPO run
    PPO_RUN_TOT_TIMESTEPS = 100 if not LOCAL_TESTING else 10000
    # PPO_RUN_TOT_TIMESTEPS = 8e6 if not LOCAL_TESTING else 10000
...
best_bc_model_paths {'train': {'simple': 'simple_bc_train_seed3', 
'random1': 'random1_bc_train_seed0', 'unident_s': 'unident_s_bc_train_seed0', 'random0': 'random0_bc_train_seed0', 
'random3': 'random3_bc_train_seed2'}, 'test': {'simple': 'simple_bc_test_seed2', 
'random1': 'random1_bc_test_seed2', 'unident_s': 'unident_s_bc_test_seed0', 
'random0': 'random0_bc_test_seed2', 'random3': 'random3_bc_test_seed3'}}

BEST EVALUATIONS
best_bc_model_evaluations {'simple': {'train': {0: 100.2, 1: 77.8, 2: 112.0, 3: 106.2, 4: 102.4}, 
'test': {0: 116.2, 1: 109.0, 2: 109.2, 3: 99.0, 4: 105.4}}, 'random1': {'train': {0: 67.4, 1: 87.2, 2: 83.2, 3: 79.4, 4: 82.6}, 'test': {0: 78.4, 1: 80.0, 2: 74.8, 3: 79.6, 4: 73.4}}, 'unident_s': {'train': {0: 137.2, 1: 129.0, 2: 115.0, 3: 138.6, 4: 136.4}, 'test': {0: 127.4, 1: 134.4, 2: 130.4, 3: 100.8, 4: 132.4}}, 'random0': {'train': {0: 31.8, 1: 24.0, 2: 25.6, 3: 16.2, 4: 34.4}, 'test': {0: 40.0, 1: 35.8, 2: 42.6, 3: 36.6, 4: 32.8}}, 'random3': {'train': {0: 42.6, 1: 47.0, 2: 43.6, 3: 42.4, 4: 45.8}, 'test': {0: 47.4, 1: 49.2, 2: 46.4, 3: 49.6, 4: 48.0}}}
...
if __name__ == "__main__":
    test_exp_params = get_params_test_exp_1_config()
    # print('test_exp_params', test_exp_params)

    evaluate_bc_in_ppo_run(test_exp_params)
