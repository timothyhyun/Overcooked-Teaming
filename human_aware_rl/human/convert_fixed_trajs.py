...
from human_aware_rl.experiments.generate_synthetic_fixed_strat_data import generate_synthetic_fixed, run_one_game_fixed_agents
...
def synthetic_dict_traj_to_python_joint_traj(team_id_to_state_action_sequences, team_id, layout_name='random0', horizon=400, complete_traj=True):

    python_layout_name = layout_name
    ...
    overcooked_states = team_id_to_state_action_sequences[team_id]['states']
    overcooked_actions = team_id_to_state_action_sequences[team_id]['actions']
    overcooked_rewards = team_id_to_state_action_sequences[team_id]['rewards']
...
def get_fixed_trajs(fixed_strat_params, N_teams=6):
    """
    Converts and returns trajectories from dataframe at `data_path` to overcooked trajectories.
    """
    (a0_type, a1_type, N_teams) = fixed_strat_params
    print(f"Generating FIXED sequences w/ params: {fixed_strat_params}")

    team_id_to_state_action_sequences = generate_synthetic_fixed(a0_type, a1_type, N_teams=N_teams)

    trajs = convert_state_action_trajs_to_overcooked_single(team_id_to_state_action_sequences, layout_name='random0')
...
def convert_state_action_trajs_to_overcooked_single(team_id_to_state_action_sequences, layout_name='random0'):
    """
    ...
    for team_id in team_id_to_state_action_sequences:
        print('TEAM ID = ', team_id)

        # Get python trajectory data and information on which player(s) was/were human
        joint_traj_data, traj_metadata = synthetic_dict_traj_to_python_joint_traj(team_id_to_state_action_sequences, team_id)

        human_idx = [0, 1]

        # Convert joint trajectories to single agent trajectories, appending recovered info to the `trajectories` dict
        joint_state_trajectory_to_single(single_agent_trajectories, joint_traj_data, traj_metadata, human_idx,
                                         processed=True)

    # print("single_agent_trajectories", single_agent_trajectories)
...
def joint_state_trajectory_to_single(trajectories, joint_traj_data, traj_metadata, player_indices_to_convert=None,
                                     processed=True):
    """
