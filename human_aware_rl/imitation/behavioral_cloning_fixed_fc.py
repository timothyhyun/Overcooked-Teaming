...
from human_aware_rl.human.convert_fixed_trajs import save_npz_file, get_fixed_trajs
...
def train_bc_agent(model_save_dir, bc_params, fixed_strat_params, num_epochs=1000, lr=1e-4, adam_eps=1e-8):
    # Extract necessary expert data and save in right format
    set_global_seed(64)

    # expert_trajs = get_fixed_trajs(**bc_params["data_params"])
    expert_trajs = get_fixed_trajs(fixed_strat_params, N_teams=2)
...
def plot_bc_run_modified(run_info, num_epochs, run_name, seed):
    xs = range(0, num_epochs, max(int(num_epochs / 10), 1))
    plt.plot(xs, run_info['train_losses'], label="train loss")
    plt.plot(xs, run_info['val_losses'], label="val loss")
    plt.plot(xs, run_info['val_accuracies'], label="val accuracy")
    plt.legend()
    plt.savefig('../../exploration_images/bc_run_fixed_' + str(run_name) + '_seed' + str(seed) + '.png')
    plt.close()
