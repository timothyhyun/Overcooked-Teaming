...
def perform_clustering_on_int_strings(data, n_clusters):
    original_data = copy.deepcopy(data)
    output_cluster_labels = [0]*data.shape[0]
    datapoint2num, num2datapoint = create_dicts(data)
    convert_with_dictionary(data, datapoint2num)
    clusters, centroids = k_means(n_clusters, data, dtw_distance)
    for i in range(n_clusters):
        ...
        for j in range(len(clusters[i])):
            # print(clusters[i][j])
            for e in range(original_data.shape[0]):
                # print("CHECKING.....", (original_data[e], clusters[i][j]))
                if np.array_equal(np.array(original_data[e]), np.array(clusters[i][j])):
                    output_cluster_labels[e] = i
                # else:

                    # print("NOT SAME.....", (original_data[e], clusters[i][j]))


    return output_cluster_labels, centroids
