...
from human_aware_rl.imitation.behavioral_cloning_joint import train_bc_agent, eval_with_benchmarking_from_saved, BC_SAVE_DIR, \
    plot_bc_run, \
...
from eval_two_fixed_bc_trains import evaluate_two_bc_models
...
def train_bc_agent_from_hh_data(layout_name, agent_name, num_epochs, lr, adam_eps, model, finetuning=False):
    ...
    model_save_dir = "joint_bc_" + layout_name + "_" + model + "_" + agent_name + "/"
    model_save_name = "joint_bc_" + layout_name + "_" + model + "_" + agent_name
    ...
    if finetuning:
        return train_bc_agent_w_finetuning(model_save_dir, bc_params, num_epochs=num_epochs, lr=lr, adam_eps=adam_eps), model_save_name
    return train_bc_agent(model_save_dir, bc_params, num_epochs=num_epochs, lr=lr, adam_eps=adam_eps), model_save_name
...
def train_bc_models_w_finetuning_on_single_strat_train(all_params, seeds):
    ...
    for params in all_params:
        for seed_idx, seed in enumerate(seeds):
            set_global_seed(seed)
            # 1. Train BC model
            model = train_bc_agent_from_hh_data(agent_name="bc_train_seed{}".format(seed), model='train', **params,
                                                finetuning=True)
            # plot_bc_run(model.bc_info, params['num_epochs'])
            plot_bc_run_modified(model.bc_info, params['num_epochs'], seed_idx, seed)
...
def train_joint_bc_models(all_params, seeds):
    """Train len(seeds) num of models for each layout"""
    for params in all_params:
        for seed_idx, seed in enumerate(seeds):
            set_global_seed(seed)
            # 1. Train BC model
            model, file_train = train_bc_agent_from_hh_data(agent_name="bc_train_seed{}".format(seed), model='train', **params,
                                                finetuning=False)
            # plot_bc_run(model.bc_info, params['num_epochs'])
            plot_bc_run_modified(model.bc_info, params['num_epochs'], seed_idx, seed, model='train')

            # 2. Test BC model
            model, file_test = train_bc_agent_from_hh_data(agent_name="bc_test_seed{}".format(seed), model='test', **params,
                                                finetuning=False)
            # plot_bc_run(model.bc_info, params['num_epochs'])
            plot_bc_run_modified(model.bc_info, params['num_epochs'], seed_idx, seed, model='test')
            reset_tf()

    return file_train, file_test


def evaluate_recent_bc_models(recent_train, recent_test, seeds):
    best_bc_model_paths = {
        'train': {
            "random0": recent_train,
        },
        'test': {
            "random0": recent_test,
        }
...
def run_all_bc_experiments():
    ...
    recent_train, recent_test = train_joint_bc_models(all_params, seeds)
    ...
    bc_bc_performance = evaluate_recent_bc_models(recent_train, recent_test, seeds)
    ...
    print("All BC models evaluation: ", bc_bc_performance)
