...
def compute_limbo_time_fc(old_trials, p1_data, p2_data, objects_data, p1_actions,
    ...
    for a in range(len(t) - 1):
        ...
        ################## INCREMENT LIMBO AND TRANSFER TIME ##################

        for obj_list_id in object_list_tracker:
            if object_list_tracker[obj_list_id]['name'] == 'onion':
                if object_list_tracker[obj_list_id]['is_in_limbo']:
                    object_list_tracker[obj_list_id]['total_limbo_time'] += 1
                if object_list_tracker[obj_list_id]['is_in_transfer']:
                    object_list_tracker[obj_list_id]['total_transfer_time'] += 1
        ...
        if p1_act == 'I':
            ...
            if p1_obj_x is None and p1_obj_x_next is not None:
                ...
                object_list_tracker[obj_count_id]['total_limbo_time'] = 0
                object_list_tracker[obj_count_id]['total_transfer_time'] = 0
                object_list_tracker[obj_count_id]['is_in_limbo'] = True
                object_list_tracker[obj_count_id]['is_in_transfer'] = False
            ...
            if p1_obj_x is not None and p1_obj_x_next is None:
                ...
                object_list_tracker[object_held_id]['is_in_transfer'] = True
        ...
        if p2_act == 'I':
            ...
            if p2_obj_x is None and p2_obj_x_next is not None:
                ...
                object_list_tracker[obj_picked_id]['is_in_transfer'] = False
        ...
        if p2_obj_x is not None and p2_obj_x_next is None:
            ...
            if p2_obj_name == 'soup' and (placed_obj_x, placed_obj_y) == (3, 4):
                ...
                if (placed_obj_x, placed_obj_y) == (3, 0):
                    object_list_tracker[object_held_id]['is_in_limbo'] = False
                    object_list_tracker[object_held_id]['is_in_transfer'] = False
    ...
    # Get transfer and limbo time from object_list_tracker
    # print('object_list_tracker', object_list_tracker)
    transfer_time = []
    limbo_time = []
    for obj_list_id in object_list_tracker:
        if object_list_tracker[obj_list_id]['name'] == 'onion':
            limbo_time.append(object_list_tracker[obj_list_id]['total_limbo_time'])
            transfer_time.append(object_list_tracker[obj_list_id]['total_transfer_time'])

    print('transfer_time ', transfer_time)
    print('limbo_time', limbo_time)

    return np.mean(limbo_time), np.mean(transfer_time)


def get_transfer_time_for_fc():
    ...
    for j in range(len(layout_trials)):
        ...
        avg_limbo_time, avg_transfer_time = compute_limbo_time_fc(old_trials, p1_data, p2_data, objects_data, p1_actions, p2_actions, name, time_elapsed)

        team_chunked_actions_data[trial_id] = {}
        team_chunked_actions_data[trial_id]['avg_limbo_time'] = avg_limbo_time
        team_chunked_actions_data[trial_id]['avg_transfer_time'] = avg_transfer_time


    avg_limbo = np.mean([team_chunked_actions_data[t]['avg_limbo_time'] for t in team_chunked_actions_data]) if len(team_chunked_actions_data) > 0 else 0
    avg_transfer = np.mean([team_chunked_actions_data[t]['avg_transfer_time'] for t in team_chunked_actions_data]) if len(team_chunked_actions_data) > 0 else 0

    return avg_limbo, avg_transfer


def compute_avg_obj_transfer_time(layout='fc'):
    if layout == 'fc':
        avg_limbo, avg_transfer = get_transfer_time_for_fc()
        print(f'avg_limbo = {avg_limbo}, avg_transfer = {avg_transfer}')
...
if __name__ == '__main__':
    compute_avg_obj_transfer_time(layout = 'fc')
