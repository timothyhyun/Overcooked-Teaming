...
from human_aware_rl.imitation.behavioral_cloning_joint import get_bc_agent_from_saved
...
def evaluate_ppo_and_bc_models_for_layout(layout, num_rounds, bc_model_paths, ppo_bc_model_paths, seeds, best=False,
    ...
    for seed_idx in range(len(seeds["bc_train"])):
        agent_ppo_bc_train, ppo_config = get_ppo_agent(ppo_bc_train_path, seeds["bc_train"][seed_idx], best=False)
...
def evaluate_two_bc_models_for_layout(layout, num_rounds, bc_model_paths, ppo_bc_model_paths, seeds, best=False,
                                          display=False):
    # evaluate_ppo_and_bc_models_for_layout(layout, num_rounds, best_bc_model_paths, ppo_bc_model_paths, seeds=seeds, best=best)
    assert len(seeds["bc_train"]) == len(seeds["bc_test"])
    ppo_bc_performance = defaultdict(lambda: defaultdict(list))
    ...
    for seed_idx in range(len(seeds["bc_train"])):
        ...
        # How well it generalizes to new agent in simulation?
        ppo_and_bc = evaluator.evaluate_agent_pair(AgentPair(agent_bc_test, agent_bc_test), num_games=num_rounds,
                                                   display=display)
        avg_ppo_and_bc = np.mean(ppo_and_bc['ep_returns'])
        ppo_bc_performance[layout]["BC_test_0+BC_test_0"].append(avg_ppo_and_bc)
        print("avg_ppo_and_bc", avg_ppo_and_bc)

        # bc_and_ppo = evaluator.evaluate_agent_pair(AgentPair(agent_bc_test, agent_bc_test), num_games=num_rounds,
        #                                            display=display)
        # avg_bc_and_ppo = np.mean(bc_and_ppo['ep_returns'])
        # ppo_bc_performance[layout]["PPO_BC_train+BC_test_1"].append(avg_bc_and_ppo)
        #
...
def evaluate_two_bc_models_for_layout2(layout, num_rounds, bc_model_paths, seeds, best=False,
                                          display=False):
    # evaluate_ppo_and_bc_models_for_layout(layout, num_rounds, best_bc_model_paths, ppo_bc_model_paths, seeds=seeds, best=best)
    assert len(seeds["bc_train"]) == len(seeds["bc_test"])
    ppo_bc_performance = defaultdict(lambda: defaultdict(list))
    ...
    for seed_idx in range(len(seeds["bc_train"])):
        ...
        # How well it generalizes to new agent in simulation?
        ppo_and_bc = evaluator.evaluate_agent_pair(AgentPair(agent_bc_train, agent_bc_test), num_games=num_rounds,
                                                   display=display)
        avg_ppo_and_bc = np.mean(ppo_and_bc['ep_returns'])
        ppo_bc_performance[layout]["BC_train+BC_test"].append(avg_ppo_and_bc)

        bc_and_ppo = evaluator.evaluate_agent_pair(AgentPair(agent_bc_test, agent_bc_train), num_games=num_rounds,
                                                   display=display)
        avg_bc_and_ppo = np.mean(bc_and_ppo['ep_returns'])
        ppo_bc_performance[layout]["BC_testn+BC_train"].append(avg_bc_and_ppo)

        ppo_and_bc = evaluator.evaluate_agent_pair(AgentPair(agent_bc_train, agent_bc_train, allow_duplicate_agents=True), num_games=num_rounds,
                                                   display=display)
        avg_ppo_and_bc = np.mean(ppo_and_bc['ep_returns'])
        ppo_bc_performance[layout]["BC_train+BC_train"].append(avg_ppo_and_bc)

        bc_and_ppo = evaluator.evaluate_agent_pair(AgentPair(agent_bc_test, agent_bc_test, allow_duplicate_agents=True), num_games=num_rounds,
                                                   display=display)
        avg_bc_and_ppo = np.mean(bc_and_ppo['ep_returns'])
        ppo_bc_performance[layout]["BC_test+BC_test"].append(avg_bc_and_ppo)
...
def run_two_fixed_bc_models_for_layout(layout, num_rounds, bc_model_paths, seeds, best=False,
                                 display=False):
    # evaluate_ppo_and_bc_models_for_layout(layout, num_rounds, best_bc_model_paths, ppo_bc_model_paths, seeds=seeds, best=best)
    # assert len(seeds["bc_train"]) == len(seeds["bc_test"])
    ...
    dp_agent_0 = DualPotAgent(simple_mdp, player_index=0)
    dp_agent_1 = DualPotAgent(simple_mdp, player_index=1)
    # dp_agent_0 = SinglePotAgent(simple_mdp, player_index=0)
    # dp_agent_1 = SinglePotAgent(simple_mdp, player_index=1)
    ...
    # visualize_trial(bc_and_bc, title="bcdp_bcdp", video_filename='bcdp_bcdp')
    ...
    bc_and_bc = evaluator.evaluate_agent_pair(
        ...
        num_games=max(int(num_rounds), 1), display=display)
    # visualize_trial(bc_and_bc, title="bcdp_fdp", video_filename='bcdp_fdp')
...
#     # assert common_keys_equal(bc_params["mdp_params"], ppo_config["mdp_params"])
#
#     orig_num_games = max(int(num_rounds), 1)
#     num_games = 4
#
#     # Play two agents against each other.
#     bc_and_bc = evaluator.evaluate_agent_pair(AgentPair(agent_bc_train, agent_bc_test, allow_duplicate_agents=False), num_games=num_games, display=display)
#     avg_bc_and_bc = np.mean(bc_and_bc['ep_returns'])
#     bc_bc_performance[layout]["BC_train+BC_test"].append(avg_bc_and_bc)
#
#     # visualize_trial(bc_and_bc, title="og_train", video_filename='og_train')
#
#     # Swap order and Play two agents against each other.
#     bc_and_bc = evaluator.evaluate_agent_pair(
#         AgentPair(agent_bc_test, agent_bc_train, allow_duplicate_agents=False),
#         num_games=num_games, display=display)
#     avg_bc_and_bc = np.mean(bc_and_bc['ep_returns'])
#     bc_bc_performance[layout]["BC_test+BC_train"].append(avg_bc_and_bc)
#
#     # visualize_trial(bc_and_bc, title="og_test", video_filename='og_test')
#
#     return bc_bc_performance
...
def run_two_bc_models_for_layout(layout, num_rounds, bc_model_paths, seeds, best=False,
    ...
    bc_bc_performance = defaultdict(lambda: defaultdict(list))
    print("INPUT: bc_model_paths", bc_model_paths)
    ...
    bc_and_bc = evaluator.evaluate_agent_pair(AgentPair(agent_bc_train, agent_bc_test, allow_duplicate_agents=False), num_games=num_games, display=display)
    avg_bc_and_bc = np.mean(bc_and_bc['ep_returns'])
    print("avg_bc_and_bc", avg_bc_and_bc)
    bc_bc_performance[layout]["BC_train+BC_test"].append(avg_bc_and_bc)
    ...
    bc_and_bc = evaluator.evaluate_agent_pair(
        ...
        num_games=num_games, display=display)
    avg_bc_and_bc = np.mean(bc_and_bc['ep_returns'])
    print("avg_bc_and_bc 2", avg_bc_and_bc)
    bc_bc_performance[layout]["BC_test+BC_train"].append(avg_bc_and_bc)
...
def evaluate_two_bc_models(best_bc_model_paths, num_rounds, seeds, best):
    ...
    for layout in layouts:
        print(layout)
        layout_eval = evaluate_two_bc_models_for_layout2(layout, num_rounds, best_bc_model_paths, seeds=seeds, best=best)
        ppo_bc_performance.update(dict(layout_eval))
    return ppo_bc_performance
...
def check_replicate_evaluate_all_ppo_bc_experiments(best_bc_model_paths):
    ...
    ppo_bc_model_paths = {
        'bc_train': {
            ...
            # "random0": "2021_08_19-13_36_51_dual_strat_wft_ppo_bc_train_random0_test5",
            # "random0": "2021_10_01-16_58_19_fixedstrat_dpdp_ppo_bc_train_random0_test3",
            # "random0": "2021_10_01-16_58_19_fixedstrat_dpdp_ppo_bc_train_random0_test3",
            "random0": "2021_09_24-00_19_30_FP_DPS_ppo_bc_train_random0_test6",
        },
        'bc_test': {
            # "random0": "orig_berk_random0_bc_test_seed2",
            ...
            # "random0": "2021_10_01-16_58_19_fixedstrat_dpdp_ppo_bc_train_random0_test3",
            # "random0": "2021_10_01-16_59_14_fixedstrat_spsp_ppo_bc_train_random0_test3",
            "random0": "2021_09_24-00_18_45_FP_SPS_ppo_bc_train_random0_test6",
        }
    }
...
def run_two_ppo_agents():
    ...
    set_global_seed(248)
    num_rounds = 100
    ppo_bc_performance = evaluate_two_ppo_models(ppo_bc_model_paths, best_bc_model_paths, num_rounds, seeds, best=False)
