...
from overcooked_ai_py.agents.fixed_strategy_agent import DualPotAgent, SinglePotAgent, FixedStrategy_AgentPair
...
def my_config():
    ...
    OTHER_AGENT_TYPE = "fixed_strategy"
...
def configure_other_agent(params, gym_env, mlp, mdp):
    ...
    if params["OTHER_AGENT_TYPE"] == "hm":
    ...
    elif params["OTHER_AGENT_TYPE"] == "fixed_strategy":
        layout_name = 'random0'
        ...
        # dp_agent_0 = DualPotAgent(simple_mdp, player_index=0, medium_level_planner=mlp)
        agent = DualPotAgent(simple_mdp, player_index=1)
        gym_env.use_action_method = True
...
def load_training_data(run_name, seeds=None):
    # run_dir = 'ppo/' + PPO_DATA_DIR + run_name + "/"
