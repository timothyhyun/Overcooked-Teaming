...
from human_aware_rl.human.process_dataframe_for_joint import save_npz_file, get_trajs_from_data, get_trajs_from_data_selective, \
...
def init_gym_env(bc_params):
    ...
    gym_env.custom_init(env, featurize_fn=lambda x: mdp.featurize_state(x, mlp, prev_actions))
    return gym_env
...
def get_bc_agent_from_model(model, bc_params, no_waits=False):
    ...
    def state_policy(mdp_states, agent_indices, prev_actions, include_waits, stochastic=False):
        ...
        encode_fn = lambda s: mdp.featurize_state(s, mlp, prev_actions)
...
def plot_bc_run_modified(run_info, num_epochs, seed_idx, seed, model):
    xs = range(0, num_epochs, max(int(num_epochs / 10), 1))
    plt.plot(xs, run_info['train_losses'], label="train loss")
    plt.plot(xs, run_info['val_losses'], label="val loss")
    plt.plot(xs, run_info['val_accuracies'], label="val accuracy")
    plt.legend()
    plt.savefig('../../exploration_images/joint_bc_run_' + model + '_seedidx' + str(seed_idx) + '_seed' + str(seed) + '.png')
    plt.close()
...
class ImitationAgentFromPolicy(AgentFromPolicy):
    ...
    def action(self, state, prev_actions):
        selected_action = self.actions(state, prev_actions)
    ...
    def actions(self, states, prev_actions, agent_indices=None):
        ...
        all_actions = self.multi_action(states, prev_actions, agent_indices)
        # print("self.prev_actions to all actions", all_actions)


        # return all_actions[self.agent_index]

        if len(agent_indices) > 1:
            return all_actions
        return all_actions[0]

    def multi_action(self, states, prev_actions, agent_indices):
        try:
            action_probs_n = list(self.state_policy(states, agent_indices, prev_actions, not self.no_waits))
        ...
        for parallel_agent_idx, curr_agent_action_probs in enumerate(action_probs_n):
            ...
            if self.stochastic:
                ...
                action_idx = np.random.choice(len(curr_agent_action_probs))
            else:
                ...
                curr_agent_action_probs = np.random.choice(len(curr_agent_action_probs))
