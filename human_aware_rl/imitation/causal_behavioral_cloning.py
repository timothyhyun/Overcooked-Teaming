...
from overcooked_ai_py.agents.agent import AgentFromPolicy, AgentPair, CausalAgentPair
...
def train_bc_agent_cross_validation(is_train, train_workers, test_workers, model_save_dir, bc_params, num_epochs=1000,
                                    lr=1e-4, adam_eps=1e-8):
    # Extract necessary expert data and save in right format
    set_global_seed(64)
    expert_trajs = get_trajs_from_data_for_cross_validation(is_train, train_workers, test_workers,
...
def train_bc_agent_w_finetuning(model_save_dir, bc_params, num_epochs=1000, lr=1e-4, adam_eps=1e-8):
    ...
    # Pass the ExpertDataset into BC model and params
    # Return the BC model
    return bc_w_finetune_from_dataset_and_params(dataset, finetune_dataset, bc_params, model_save_dir, num_epochs, lr,
                                                 adam_eps)
...
def bc_w_finetune_from_dataset_and_params(dataset, finetune_dataset, bc_params, model_save_dir, num_epochs, lr,
                                          adam_eps):
    # Setup env
    gym_env = init_gym_env(bc_params)
    ...
    model = GAIL("MlpPolicy", gym_env, dataset, verbose=1)
    # Only pretrain the GAIL model, which is just supervised BC on the ExpertDataset.
    model.pretrain_and_finetune(dataset, finetune_dataset, n_epochs=num_epochs, learning_rate=lr, adam_epsilon=adam_eps,
                                save_dir=BC_SAVE_DIR + model_save_dir)
...
def get_bc_agent_from_model(model, bc_params, no_waits=False):
    ...
    def encoded_state_policy(observations, include_waits=True, stochastic=False):
        ...
        if not include_waits:
            action_probs = Active_Causal_ImitationAgentFromPolicy.remove_indices_and_renormalize(action_probs_n, [
    ...
    return Active_Causal_ImitationAgentFromPolicy(state_policy, encoded_state_policy, no_waits=no_waits, mlp=mlp)
...
def plot_training_loss(training_losses):
    ...
    plt.savefig("imgs_causal/training_loss_1.png")
    plt.close()

class Active_Causal_ImitationAgentFromPolicy(AgentFromPolicy):
    ...
    def __init__(self, state_policy, direct_policy, mlp=None, stochastic=True, no_waits=False, stuck_time=3):
        ...
        self.past_overcooked_states = []
        self.past_agent_actions = []
        self.past_prev_actions = []

        # Make Partner Policy the same as your policy
        self.make_partner_policy()

    def make_partner_policy(self):
        ...
        self.partner_policy = BasicNet().double()
        self.mdp = OvercookedGridworld.from_layout_name('random0')
        self.mlp = MediumLevelPlanner.from_pickle_or_compute(self.mdp, NO_COUNTERS_PARAMS, force_compute=False)
        self.previous_state = None
        self.agent_action_in_previous_state = None
        self.agent_index = 0
        ...
        self.optimizer = optim.SGD(self.partner_policy.parameters(), lr=self.lr, momentum=self.momentum)
        ...
        self.partner_policy.train()

    def create_dataset_for_partner_policy(self, previous_overcooked_state, agent_action_in_previous_state, prev_actions):
        ordered_features_p0, ordered_features_p1 = self.mdp.featurize_state(previous_overcooked_state, self.mlp)
        if self.partner_index == 0:
            input_features = ordered_features_p0
        else:
            input_features = ordered_features_p1

        # print("input_features shape", input_features.shape)
        # print('agent_action_in_previous_state[self.agent_index]', agent_action_in_previous_state)
        # input_features = np.concatenate(input_features, list(agent_action_in_previous_state))
        partner_action = Action.ACTION_TO_INDEX[prev_actions[self.partner_index]]
        # print("partner_action", partner_action)
        true_label_partner_action = np.array([partner_action])



        input_features = torch.from_numpy(np.expand_dims(input_features, axis=0))
        true_label_partner_action = torch.from_numpy(true_label_partner_action)

        # print("input_features shape", input_features.shape)
        # print("true_label_partner_action", true_label_partner_action.shape)

        return input_features, true_label_partner_action

    def train_partner_policy(self, input_features, true_label_partner_action):
        batch_data = input_features.double()
        batch_labels = true_label_partner_action

        predicted_output = self.partner_policy(batch_data)

        predicted_output = predicted_output.double()
        target_output = batch_labels


        # print("PARTNER POLICY PREDICTIONS, TARGETS: ", (predicted_output, target_output))
        ...
        predicted_output = np.argmax(predicted_output.detach().numpy())
        target_output = target_output.detach().numpy()[0]
        if predicted_output == target_output:
            self.n_correct.append(1)
        else:
            self.n_correct.append(0)

        if len(self.n_correct) >= 200 and len(self.n_correct)%100==0:
            self.accuracies.append(sum(self.n_correct[-100:])/len(self.n_correct[-100:]))



    def update_accuracy(self):
        num_correct = 0
        for i in range(len(self.past_overcooked_states)):
            overcooked_state = self.past_overcooked_states[i]
            agent_action_in_previous_state = self.past_agent_actions[i]
            prev_actions = self.past_prev_actions[i]
            input_features, true_label_partner_action = self.create_dataset_for_partner_policy(overcooked_state,
                                                                                               agent_action_in_previous_state,
                                                                                               prev_actions)
            ...
            target_output = target_output.detach().numpy()[0]
            # loss = self.loss_fn(predicted_output, target_output)
            # print("pred, target", (predicted_output, target_output))
            if predicted_output == target_output:
                num_correct += 1
            # num_correct += loss.item()

        accuracy = num_correct/len(self.past_overcooked_states)
        self.accuracies.append(accuracy)


    def update_partner_policy(self, overcooked_states, agent_action_in_previous_states, prev_actions):
        for i in range(len(overcooked_states)):
            overcooked_state = overcooked_states[i]
            agent_action_in_previous_state = agent_action_in_previous_states[i]
            input_features, true_label_partner_action = self.create_dataset_for_partner_policy(overcooked_state, agent_action_in_previous_state, prev_actions)

            self.past_overcooked_states.append(overcooked_state)
            self.past_agent_actions.append(agent_action_in_previous_state)
            self.past_prev_actions.append(prev_actions)

            self.train_partner_policy(input_features, true_label_partner_action)




    def action(self, state, prev_actions):




        selected_actions = self.actions(state, prev_actions)


        return selected_actions

    def actions(self, states, prev_actions, agent_indices=None):
        """
        ...
        if self.previous_state is not None and self.agent_action_in_previous_state is not None:
            self.update_partner_policy(self.previous_state, self.agent_action_in_previous_state, prev_actions)

        # print("all_actions", all_actions)
        self.previous_state = states
        self.agent_action_in_previous_state = all_actions
    ...
    def multi_action(self, states, agent_indices):
        ...
        for parallel_agent_idx, curr_agent_action_probs in enumerate(action_probs_n):
            ...
            if self.stochastic:
                ...
                if action_idx == np.argmax(curr_agent_action_probs) and np.random.uniform() > 0.3:
                    action_idx = np.argmin(curr_agent_action_probs)
            else:
