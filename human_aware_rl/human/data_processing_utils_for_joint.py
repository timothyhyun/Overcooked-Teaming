...
def df_traj_to_python_joint_traj(traj_df, complete_traj=True):
    ...
    assert sum(
        overcooked_rewards) == datapoint.reward_norm_total, "Rewards didn't sum up to cumulative rewards. Probably trajectory df is corrupted / not complete"
...
def convert_joint_df_trajs_to_overcooked_single(main_trials, worker_ids, layout_names, ordered_pairs=True,
...
def joint_state_trajectory_to_single(trajectories, joint_traj_data, traj_metadata, player_indices_to_convert=None,
    ...
    for agent_idx in player_indices_to_convert:
        ...
        for i in range(len(states)):
            ...
            if i-1 < 0:
                prev_joint_actions = joint_actions[i-1]
            else:
                prev_joint_actions = [(0,0), (0,0)]
            ...
            if processed:
                ...
                state = mdp.featurize_state(state, mlp, prev_joint_actions)[agent_idx]
