...
def get_state_tuple(old_trials, p1_data, p2_data, objects_data, p1_actions,
    ...
    for a in range(len(t) - 1):
        ...
        p1_hold_action = ['None', 'None']
        p2_hold_action = ['None', 'None']
        ...
        state_tuples_add = (p1_hold_action[0], p1_hold_action[1], p2_hold_action[0], p2_hold_action[1],
...
def featurize_data_for_granular_hmm():
    ...
    for j in range(len(layout_trials)):
        ...
        state_tuple_list = get_state_tuple(old_trials,
                                        p1_data,
                                        p2_data,
                                        objects_data,
                                        p1_actions,
        ...
        team_state_data[trial_id] = state_tuple_list
    ...
    #     print()
    return team_state_data, team_numbers
...
def recalibrate_featurized_data(team_chunked_actions_data):
    ...
    for i in range(X_data.shape[0]):
        ...
        X_data_new.append(np.array(add)[:10])
...
def reindex_state_tuples(team_state_data):
    ...
    for trial_id in team_state_data:
        all_state_tuples.extend(team_state_data[trial_id])
    unique_state_tuples = list(set(all_state_tuples))
...
def run_granular_hmm():
    ...
    # X = observation_data
    # Y = hidden_state_data
    team_state_data, team_numbers = featurize_data_for_granular_hmm()
    X = reindex_state_tuples(team_state_data)
