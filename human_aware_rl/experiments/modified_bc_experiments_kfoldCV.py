...
from human_aware_rl.imitation.behavioural_cloning import train_bc_agent, eval_with_benchmarking_from_saved,BC_SAVE_DIR, plot_bc_run, \
    DEFAULT_BC_PARAMS, get_bc_agent_from_saved, plot_bc_run_modified, train_bc_agent_w_finetuning, train_bc_agent_cross_validation
import itertools
...
def train_bc_agent_from_hh_data(layout_name, agent_name, num_epochs, lr, adam_eps, model, finetuning=False):
    ...
    bc_params["data_params"]['data_path'] = "../data/human/anonymized/clean_main_trials.pkl"
...
def train_bc_agent_from_hh_data_for_cross_validation(cv_index, is_train, train_workers, test_workers, layout_name, agent_name, num_epochs, lr, adam_eps, model, finetuning=False):
    """Trains a BC agent from human human data (model can either be `train` or `test`, which is trained
    on two different subsets of the data)."""
    ...
    model_save_dir = "dp_crossval_" +str(cv_index)+"_"+ layout_name + "_" + agent_name + "/"
    # train_bc_agent() returns the bc model
    # if finetuning:
    #     return train_bc_agent_w_finetuning(model_save_dir, bc_params, num_epochs=num_epochs, lr=lr, adam_eps=adam_eps)
    return train_bc_agent_cross_validation(is_train, train_workers, test_workers, model_save_dir, bc_params, num_epochs=num_epochs, lr=lr, adam_eps=adam_eps)
...
def train_bc_models_dual_strat_cross_validation(cv_index, train_workers, test_workers, all_params, seeds):
    """Train len(seeds) num of models for each layout"""
    for params in all_params:
        for seed_idx, seed in enumerate(seeds):
            set_global_seed(seed)
            # 1. Train BC model
            model = train_bc_agent_from_hh_data_for_cross_validation(cv_index, True, train_workers, test_workers, agent_name="bc_train_seed{}".format(seed), model='train', **params, finetuning=False)
            # plot_bc_run(model.bc_info, params['num_epochs'])
            plot_bc_run_modified(model.bc_info, params['num_epochs'], seed_idx, seed)

            # 2. Test BC model
            model = train_bc_agent_from_hh_data_for_cross_validation(cv_index, False, train_workers, test_workers, agent_name="bc_test_seed{}".format(seed), model='test', **params, finetuning=False)
            # plot_bc_run(model.bc_info, params['num_epochs'])
            plot_bc_run_modified(model.bc_info, params['num_epochs'], seed_idx, seed)
            reset_tf()
...
def run_all_cv_bc_experiments():
    ...
    # train_workers = [15, 22]
    all_other_workers = [2, 4, 17, 19, 1, 3, 10, 11, 12, 13]

    K = 4
    cval_results = {}
    counter = 0
    all_subsets = list(itertools.combinations(all_other_workers, K))
    random_selection = np.random.choice(list(range(len(all_subsets))), size=20)
    for rand_idx in random_selection:
        subset = all_subsets[rand_idx]
        print('\nsubset', subset)
        train_workers = []
        test_workers = []
        cval_results[counter] = {}
        cval_results[counter]['subset'] = subset
        for elem in all_other_workers:
            if elem in subset:
                train_workers.append(elem)
            else:
                test_workers.append(elem)

        cval_results[counter]['train_workers'] = train_workers
        cval_results[counter]['test_workers'] = test_workers

        cv_index = counter

        train_bc_models_dual_strat_cross_validation(cv_index, train_workers, test_workers, all_params, seeds)
        ...
        for layout_name, seed_indices in selected_models.items():
            ...
            final_bc_model_paths["train"][layout_name] = "dp_crossval_{}_{}_bc_train_seed{}".format(cv_index, layout_name, train_idx)
            final_bc_model_paths["test"][layout_name] = "dp_crossval_{}_{}_bc_test_seed{}".format(cv_index, layout_name, test_idx)
        #
        ...
        cval_results[counter]['best_bc_models_performance'] = best_bc_models_performance

        counter += 1


    print('\n\n\n')
    print('cval_results = ', cval_results)
    pickle.dump(cval_results, open('cval_results_n10.pkl', 'wb'))
    

if __name__ == "__main__":
    run_all_cv_bc_experiments()
