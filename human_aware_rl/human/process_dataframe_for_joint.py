...
from human_aware_rl.human.data_processing_utils_for_joint import convert_joint_df_trajs_to_overcooked_single, \
...
def get_trajs_from_data_for_cross_validation(is_train, train_workers, test_workers, data_path, train_mdps,
                                             ordered_trajs, human_ai_trajs):
    """
    Converts and returns trajectories from dataframe at `data_path` to overcooked trajectories.
    """
    print("Loading data from {}".format(data_path))
...
def get_trajs_from_data_specify_groups(selective, is_train, train_workers, test_workers, data_path, train_mdps,
                                       ordered_trajs, human_ai_trajs):
    """
    Converts and returns trajectories from dataframe at `data_path` to overcooked trajectories.
    """
...
def format_hh_trials_df(trials, clip_400):
    ...
    main_trials = main_trials.join(
        main_trials.groupby(['workerid_num', 'round_num', 'layout_name'])['cur_gameloop'].count(),
        on=['workerid_num', 'round_num', 'layout_name'], rsuffix='_total')

    # Calculate total reward for each round
    main_trials = main_trials.join(
        main_trials.groupby(['workerid_num', 'round_num', 'layout_name'])['reward_norm'].sum(),
        on=['workerid_num', 'round_num', 'layout_name'], rsuffix='_total')
